...
function subscribeToBookings(uid) {
    ...
    const { collection, query, where, onSnapshot, orderBy } = window.fbFunctions;
    const q = query(
        collection(db, 'jobs'),
                ...
                contentHtml += `
                    <div class="active-booking-card" style="min-width: 320px; max-width: 350px; flex-shrink: 0; position: relative; padding: 1.25rem; border-radius: 16px; background: rgba(255, 255, 255, 0.03); border: 1px solid var(--glass-border);">
                        ...
                        <div style="display: flex; gap: 0.5rem; border-top: 1px solid var(--glass-border); padding-top: 1rem;">
                            ...
                            <button class="btn btn-secondary btn-sm" style="flex:1; font-size:0.8rem; padding:0.5rem;" onclick="window.location.href='/pages/chat/chat.html?bookingId=${activeBooking.id}'"><i class="fas fa-comment-dots"></i></button>
    ...
    try {
       ...
       `;
        ...
        const cardsHtml = filtered.length > 0 ? filtered.map(b => {
            ...
            const chatAction = isCancelled ? 'onclick="alert(\'Cannot chat on cancelled bookings.\')"' : `onclick="window.location.href='/pages/chat/chat.html?bookingId=${b.id}&workerName=${encodeURIComponent(workerName)}'"`;
...
function openBookingPage(serviceType, workerId = null) {
    ...
    if (workerId) {
        Storage.set('karyasetu_selected_worker_id', workerId);
        console.log('Saved worker ID to storage:', workerId);
    } else {
        Storage.remove('karyasetu_selected_worker_id');
    }
